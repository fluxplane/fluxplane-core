# ghcr.io/fluxplane/fluxplane-go: minimal runtime base for Fluxplane Go apps.
#
# Contains no Fluxplane source — just the runtime userland every app needs:
# CA certificates, timezone data, and a non-root user.
# Apps FROM this image and add their statically-linked binary on top.
FROM alpine:3.20
