...
// ConfigDecoder is implemented by plugin registrations that want contributions to
...
func ConfigSchemaFor[T any]() ([]byte, error) {
	...
	if schema == nil {
		return nil, fmt.Errorf("contributions: config schema is nil")
	}
	schema.Version = invjsonschema.Version
	data, err := json.Marshal(schema)
	if err != nil {
		return nil, fmt.Errorf("contributions: marshal config schema: %w", err)
	}
	return data, nil
}
...
func ConfigAs[T any](ctx Context) (T, error) {
	...
	var zero T
	if ctx.Config == nil {
		return zero, fmt.Errorf("contributions: plugin config was not decoded")
	}
	return zero, fmt.Errorf("contributions: plugin config has type %T", ctx.Config)
}

// PrepareContext returns ctx with Config populated when plugin declares a
// ConfigDecoder.
func PrepareContext(ctx context.Context, plugin Provider, pluginCtx Context) (Context, error) {
