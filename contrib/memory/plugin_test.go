...
func TestPluginOperationsUseHybridMemoryStore(t *testing.T) {
	...
	ops, err := New().Operations(ctx, contributions.Context{
...
func TestMemoryDatasourceSearchGetAndCorpusAreScoped(t *testing.T) {
	...
	ops, err := New().Operations(ctxA, contributions.Context{EventStore: events, DataStore: data})
	if err != nil {
		t.Fatalf("Operations: %v", err)
	}
	...
	providers, err := New().DatasourceProviders(ctxA, contributions.Context{EventStore: events, DataStore: data})
	if err != nil {
