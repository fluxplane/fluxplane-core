...
func (Plugin) Manifest() contributions.Manifest {
	return contributions.Manifest{Name: Name, Description: "Scoped structured memory operations."}
}

func (Plugin) Contributions(_ context.Context, ctx contributions.Context) (resource.ContributionBundle, error) {
...
func (Plugin) EnvironmentObservers(_ context.Context, ctx contributions.Context) ([]runtimeevidence.Observer, error) {
...
func (Plugin) DatasourceProviders(_ context.Context, ctx contributions.Context) ([]coredatasource.Provider, error) {
	store, err := runtimememory.NewStore(ctx.EventStore, ctx.DataStore)
	if err != nil {
		return nil, err
	}
