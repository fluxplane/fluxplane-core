...
const (
	// Name identifies the workspace contribution provider.
	Name = "workspace"
	// SummaryProvider identifies the auto-context provider that renders workspace roots.
	SummaryProvider = "workspace.summary"
)

// Provider contributes generic workspace context available to all local apps.
type Provider struct {
	workspace runtimeworkspace.Workspace
}

var _ contributions.Provider = Provider{}
var _ contributions.ContextProviderProvider = Provider{}

// New returns a workspace contribution provider bound to a runtime workspace.
func New(workspace runtimeworkspace.Workspace) Provider {
	return Provider{workspace: workspace}
}

// Manifest returns provider metadata.
func (Provider) Manifest() contributions.Manifest {
	return contributions.Manifest{Name: Name, Description: "Runtime workspace context."}
}

// Contributions returns workspace context provider specs.
func (Provider) Contributions(context.Context, contributions.Context) (resource.ContributionBundle, error) {
...
func (p Provider) ContextProviders(context.Context, contributions.Context) ([]corecontext.Provider, error) {
	if p.workspace == nil {
		return nil, nil
	}
