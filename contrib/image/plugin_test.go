...
func TestPluginContributesImageOperationsAndActionToolSet(t *testing.T) {
	bundle, err := New(nil).Contributions(context.Background(), contributions.Context{})
	if err != nil {
...
func TestProvidersOperationReportsMissingAndKeylessProviders(t *testing.T) {
	...
	ops, err := New(sys).Operations(context.Background(), contributions.Context{})
	if err != nil {
...
func TestGenerateOperationUsesRequestedProvider(t *testing.T) {
	...
	ops, err := plugin.Operations(context.Background(), contributions.Context{})
	if err != nil {
		t.Fatalf("Operations: %v", err)
	}
