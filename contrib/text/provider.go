package text

import (
	"context"
	"encoding/json"
	"fmt"
	"strings"

	"github.com/fluxplane/fluxplane-core/core/resource"
	"github.com/fluxplane/fluxplane-core/orchestration/contributions"
	"github.com/fluxplane/fluxplane-operation"
)

const Name = "text"

// Provider contributes deterministic text transformation operations.
type Provider struct{}

var _ contributions.Provider = Provider{}
var _ contributions.OperationProvider = Provider{}

// New returns the text contribution provider.
func New() Provider { return Provider{} }

// Config configures the text contribution provider.
type Config struct {
	// Operations selects the operations contributed by this plugin. Empty means all.
	Operations []string `json:"operations,omitempty"`
}

type textOperation struct {
	key         string
	description string
	transform   func(string) string
}

var operations = []textOperation{
	{key: "upper", description: "Convert text to uppercase.", transform: strings.ToUpper},
	{key: "lower", description: "Convert text to lowercase.", transform: strings.ToLower},
	{key: "trim", description: "Trim surrounding whitespace.", transform: strings.TrimSpace},
}

// Manifest returns provider metadata.
func (Provider) Manifest() contributions.Manifest {
	return contributions.Manifest{
		Name:        Name,
		Description: "Pure text transformation operations.",
	}
}

// Contributions returns operation specs.
func (Provider) Contributions(_ context.Context, ctx contributions.Context) (resource.ContributionBundle, error) {
	selected, err := selectedOperations(ctx.Ref)
	if err != nil {
		return resource.ContributionBundle{}, err
	}
	bundle := resource.ContributionBundle{}
	for _, op := range selected {
		spec := op.spec()
		bundle.Operations = append(bundle.Operations, spec)
	}
	return bundle, nil
}

// Operations returns executable operation implementations.
func (Provider) Operations(_ context.Context, ctx contributions.Context) ([]operation.Operation, error) {
	selected, err := selectedOperations(ctx.Ref)
	if err != nil {
		return nil, err
	}
	out := make([]operation.Operation, 0, len(selected))
	for _, selected := range selected {
		op := selected
		out = append(out, operation.New(op.spec(), func(_ operation.Context, input operation.Value) operation.Result {
			text, ok := inputText(input)
			if !ok {
				return operation.Failed("invalid_text_input", "text operation input must be a string", nil)
			}
			return operation.OK(op.transform(text))
		}))
	}
	return out, nil
}

func selectedOperations(ref resource.PluginRef) ([]textOperation, error) {
	cfg, err := decodeConfig(ref.Config)
	if err != nil {
		return nil, err
	}
	if len(cfg.Operations) == 0 {
		return append([]textOperation(nil), operations...), nil
	}
	index := make(map[string]textOperation, len(operations)*2)
	for _, op := range operations {
		index[op.key] = op
		index[string(op.ref().Name)] = op
	}
	selected := make([]textOperation, 0, len(cfg.Operations))
	seen := map[string]struct{}{}
	for _, raw := range cfg.Operations {
		key := strings.TrimSpace(raw)
		op, ok := index[key]
		if !ok {
			return nil, fmt.Errorf("text surface: unknown operation %q", raw)
		}
		if _, exists := seen[op.key]; exists {
			continue
		}
		seen[op.key] = struct{}{}
		selected = append(selected, op)
	}
	return selected, nil
}

func decodeConfig(raw map[string]any) (Config, error) {
	if len(raw) == 0 {
		return Config{}, nil
	}
	for key := range raw {
		if key != "operations" {
			return Config{}, fmt.Errorf("text surface: unknown config key %q", key)
		}
	}
	data, err := json.Marshal(raw)
	if err != nil {
		return Config{}, fmt.Errorf("text surface: encode config: %w", err)
	}
	var cfg Config
	if err := json.Unmarshal(data, &cfg); err != nil {
		return Config{}, fmt.Errorf("text surface: decode config: %w", err)
	}
	return cfg, nil
}

func (op textOperation) ref() operation.Ref {
	return operation.Ref{Name: operation.Name(op.key)}
}

func (op textOperation) spec() operation.Spec {
	return operation.Spec{
		Ref:         op.ref(),
		Description: op.description,
		Input: operation.Type{
			Name:        "text",
			Description: "Input text.",
		},
		Output: operation.Type{
			Name:        "text",
			Description: "Transformed text.",
		},
		Semantics: operation.Semantics{
			Determinism: operation.DeterminismDeterministic,
			Effects:     operation.EffectSet{operation.EffectNone},
			Idempotency: operation.IdempotencyIdempotent,
			Risk:        operation.RiskLow,
		},
	}
}

func inputText(input operation.Value) (string, bool) {
	switch v := input.(type) {
	case string:
		return v, true
	case []byte:
		return string(v), true
	case fmt.Stringer:
		return v.String(), true
	default:
		return "", false
	}
}
