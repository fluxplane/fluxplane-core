...
func jiraSearchIssues(ctx context.Context, sys system.System, session atlassianplugin.Session, jql string, startAt, maxResults int, out any) (int, error) {
	...
	return atlassianplugin.DoJSON(ctx, sys, session, http.MethodGet, "/search/jql?"+query.Encode(), nil, out)
}

func jiraGetIssue(ctx context.Context, sys system.System, session atlassianplugin.Session, key string) (Issue, error) {
	var raw jiraIssue
	if _, err := atlassianplugin.DoJSON(ctx, sys, session, http.MethodGet, "/issue/"+url.PathEscape(strings.TrimSpace(key))+"?expand=renderedFields,names", nil, &raw); err != nil {
		return Issue{}, err
	}
	return issueFromJira(raw), nil
}

func jiraListProjects(ctx context.Context, sys system.System, session atlassianplugin.Session, startAt, maxResults int) ([]Project, int, error) {
	query := url.Values{}
	query.Set("startAt", strconv.Itoa(max(0, startAt)))
	query.Set("maxResults", strconv.Itoa(normalizedLimit(maxResults)))
	var data jiraProjectSearchResponse
	if _, err := atlassianplugin.DoJSON(ctx, sys, session, http.MethodGet, "/project/search?"+query.Encode(), nil, &data); err != nil {
		return nil, 0, err
	}
...
func jiraGetProject(ctx context.Context, sys system.System, session atlassianplugin.Session, key string) (Project, error) {
	var raw jiraProject
	if _, err := atlassianplugin.DoJSON(ctx, sys, session, http.MethodGet, "/project/"+url.PathEscape(strings.TrimSpace(key)), nil, &raw); err != nil {
		return Project{}, err
	}
...
func (p Plugin) authzBaseURL(ctx context.Context) string {
	cfg := atlassianplugin.NormalizeConfig(p.cfg)
	if cfg.BaseURL != "" {
		return cfg.BaseURL
	}
	if cfg.CloudID != "" {
		return atlassianplugin.BaseURL(AtlassianProduct(), cfg.CloudID)
	}
	if stored, ok, err := p.store.LoadSecret(ctx, atlassianplugin.OAuthSecretRef(Name, p.ref)); err == nil && ok {
		if cloudID := strings.TrimSpace(stored.Metadata["cloud_id"]); cloudID != "" {
			return atlassianplugin.BaseURL(AtlassianProduct(), cloudID)
		}
	}
