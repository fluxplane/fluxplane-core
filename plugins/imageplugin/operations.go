...
func (p Plugin) generate(ctx operation.Context, req GenerateRequest) operation.Result {
	...
	provider, err := selectGenerationProvider(p.system, p.generators, req.Provider)
	if err != nil {
		return operation.Failed("image_generate_provider_unavailable", err.Error(), map[string]any{"providers": p.providerOutput()})
	}
	result, err := provider.Generate(ctx, p.system, req)
	if err != nil {
		return operation.Failed("image_generate_failed", err.Error(), map[string]any{"provider": provider.Info(p.system).Name})
	}
...
func (p Plugin) understand(ctx operation.Context, req UnderstandRequest) operation.Result {
	...
	provider, err := selectUnderstandingProvider(p.system, p.understanders, req.Provider)
	if err != nil {
		return operation.Failed("image_understand_provider_unavailable", err.Error(), map[string]any{"providers": p.providerOutput()})
	}
	result, err := provider.Understand(ctx, p.system, req)
	if err != nil {
		return operation.Failed("image_understand_failed", err.Error(), map[string]any{"provider": provider.Info(p.system).Name})
	}
...
func (p Plugin) providers(operation.Context, providersInput) operation.Result {
	out := p.providerOutput()
	var lines []string
...
func (p Plugin) providerOutput() providersOutput {
	out := providersOutput{}
	for _, provider := range p.generators {
		if provider != nil {
			out.Generation = append(out.Generation, provider.Info(p.system))
		}
	}
	for _, provider := range p.understanders {
		if provider != nil {
			out.Understanding = append(out.Understanding, provider.Info(p.system))
		}
	}
