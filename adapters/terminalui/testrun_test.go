...
func TestRenderTestRunEvent(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "build failed",
			event: testrun.Event{Target: "./adapters/sqleventstore", Status: testrun.StatusFailed, Summary: testrun.Summary{PackagesFailed: 1}, Failures: []testrun.Failure{{Kind: testrun.FailureBuild, File: "runtime/thread/store.go", Line: 171, Column: 15, Message: "undefined: threadIdempotencyKey"}}, DurationMS: 310},
			want:  []string{"🛠️ tests failed to build  ./adapters/sqleventstore", "store.go:171 undefined: threadIdempotencyKey", "1 package failed"},
		},
		{
