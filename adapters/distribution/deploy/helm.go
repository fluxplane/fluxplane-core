...
func writeHelmChart(loaded distribution.Loaded, output, image, release, namespace, imagePullPolicy, envSecretName, authPath string, appRuntime appRuntimeOptions, nodeSelectors []string, values map[string]string, dryRun, force bool, out io.Writer) ([]string, error) {
	...
	files := map[string]string{
		...
		filepath.Join(output, "values.yaml"):           helmValuesYAML(repository, tag, namespace, chartEnvSecretName, rendered.SecretName != "", values),
		filepath.Join(output, "templates", "app.yaml"): helmTemplateAppYAML(rendered.Content, rendered.SecretName != ""),
	}
...
func helmValuesYAML(repository, tag, namespace, envSecretName string, envSecretEnabled bool, values map[string]string) string {
...
func helmTemplateAppYAML(content string, hasEnvSecret bool) string {
	if !hasEnvSecret {
		return content
	}
