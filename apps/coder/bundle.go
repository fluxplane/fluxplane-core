...
import (
	...
	"github.com/fluxplane/engine/plugins/native/code"
	...
	"github.com/fluxplane/engine/sdk"
)
...
For repository, documentation, or review-corpus investigations, start with bounded discovery and targeted reads. Prefer a small dir_list/project_files pass, then narrow file_read ranges or path-scoped grep queries over broad repository-wide searches. If a tool result is replaced, truncated, or repaired as missing, treat that as a signal to narrow the next call immediately instead of repeating another broad query.
